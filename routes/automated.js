...
router.get("/selectDesign", function(req, res) {
    if (req.session.userDesign === undefined || req.session.userDesign === null) {
        res.redirect("/");
    }
    firebase
        .database()
        ...
        .once("value")
        .then((d) => {
            // res.json({ type: req.session.userDesign.designType, data: d.val() });
            if (req.session.userDesign.designType === "MedicalCaps") {
                res.render("pages/automated/selectMedicalCaps", {
                    action: "selectDesign",
                    user: req.session,
                    designs: d,
                });
