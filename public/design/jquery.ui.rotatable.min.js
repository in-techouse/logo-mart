(function (c) {
  ...
  c.widget("ui.rotatable", c.ui.mouse, {
    ...
    angle: function (a) {
      if (void 0 === a) return this.options.angle;
      this.elementCurrentAngle = this.options.angle = a;
      this._performRotation(this.options.angle);
    },
    getElementCenter: function () {
      null == this.elementCenter &&
        (this.elementCenter = this._calculateElementCenter());
      return this.elementCenter;
    },
    handle: function (a) {
      if (void 0 === a) return this.options.handle;
      this.options.handle = a;
    },
    plugins: {},
    rotationCenterOffset: function (a) {
      if (void 0 === a) return this.options.rotationCenterOffset;
      null !== a.top && (this.options.rotationCenterOffset.top = a.top);
      null !== a.left && (this.options.rotationCenterOffset.left = a.left);
    },
    rotateElement: function (a) {
      if (!this.element || this.element.disabled || this.options.disabled)
        return !1;
      if (!a.which) return this.stopRotate(a), !1;
      var b = this._calculateRotateAngle(a),
        c = this.elementCurrentAngle;
      this.elementCurrentAngle = b;
      this._propagate("rotate", a);
      if (!1 === this._propagate("rotate", a))
        return (this.elementCurrentAngle = c), !1;
      var d = this.ui();
      if (!1 === this._trigger("rotate", a, d))
        return (this.elementCurrentAngle = c), !1;
      d.angle.current !== b && (this.elementCurrentAngle = b = d.angle.current);
      this._performRotation(b);
      c !== b && (this.hasRotated = !0);
      return !1;
    },
    startRotate: function (a) {
      var b = this.getElementCenter();
      this.mouseStartAngle = Math.atan2(a.pageY - b.y, a.pageX - b.x);
    ...
    stopRotate: function (a) {
      if (this.element && !this.element.disabled)
        return (
          c(document).unbind("mousemove", this.listeners.rotateElement),
    ...
    wheelRotate: function (a) {
      var b = this._angleInRadians(Math.round(a.originalEvent.deltaY / 10));
    ...
    _angleInRadians: function (a) {
      return (a * Math.PI) / 180;
    },
    _angleInDegrees: function (a) {
      return (180 * a) / Math.PI;
    },
    _calculateElementCenter: function () {
      var a = this._getElementOffset();
      if (this._isRotationCenterSet())
        return {
          x: a.left + this.rotationCenterOffset().left,
          y: a.top + this.rotationCenterOffset().top,
        };
      if (void 0 !== this.element.css("transform-origin")) {
        var b = this.element
          ...
          .match(/([\d.]+)px +([\d.]+)px/);
        if (null != b)
          return { x: a.left + parseFloat(b[1]), y: a.top + parseFloat(b[2]) };
      }
      return {
        x: a.left + this.element.width() / 2,
        y: a.top + this.element.height() / 2,
      };
    },
    _calculateSnap: function (a) {
      a = this._angleInDegrees(a);
      a = Math.round(a / this.options.step) * this.options.step;
      return this._angleInRadians(a);
    },
    _calculateRotateAngle: function (a) {
      var b = this.getElementCenter(),
        b =
          Math.atan2(a.pageY - b.y, a.pageX - b.x) -
    ...
    _create: function () {
      var a;
      if (this.options.handle) a = this.options.handle;
      else if (
        ...
        a.css("position", "relative"),
          a.css("top", this.options.handleOffset.top + "px"),
          a.css("left", this.options.handleOffset.left + "px");
      this.listeners = {
      ...
      a.draggable({ helper: "clone", start: this._dragStart, handle: a });
    ...
    _getTransforms: function (a) {
      a = "rotate(" + a + "rad)";
      if (this.options.transforms) {
        var b,
          c = this.options.transforms,
          d = [];
        for (b in c)
          c.hasOwnProperty(b) && c[b] && d.push(b + "(" + c[b] + ")");
        b = d.join(" ");
        a += " " + b;
      }
      return a;
    ...
    _performRotation: function (a) {
      ...
      a = this._getTransforms(a);
      this.element.css("transform", a);
      this.element.css("-moz-transform", a);
      this.element.css("-webkit-transform", a);
      this.element.css("-o-transform", a);
    },
